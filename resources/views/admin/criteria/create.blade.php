            ...
            <form action="{{ route('admin.criteria.store') }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <input type="text" id="kode" name="kode" value="{{ old('kode') }}" placeholder="Contoh: C1" required
                    ...
                    <div>
                        ...
                        <select id="nama_kriteria" name="nama_kriteria" required
                            ...
                            <option value="" disabled {{ old('nama_kriteria') ? '' : 'selected' }}>Pilih Kriteria Baku...</option>

                            <optgroup label="--- Kategori Nutrisi & Gizi ---" class="font-bold text-saku-primary">
                                <option value="Kandungan Protein" {{ old('nama_kriteria') == 'Kandungan Protein' ? 'selected' : '' }}>Kandungan Protein</option>
                                <option value="Kandungan Karbohidrat / Kalori" {{ old('nama_kriteria') == 'Kandungan Karbohidrat / Kalori' ? 'selected' : '' }}>Kandungan Karbohidrat / Kalori</option>
                                <option value="Kandungan Serat / Sayur" {{ old('nama_kriteria') == 'Kandungan Serat / Sayur' ? 'selected' : '' }}>Kandungan Serat / Sayur</option>
                            </optgroup>

                            <optgroup label="--- Kategori Kepuasan & Kualitas ---" class="font-bold text-saku-primary mt-2">
                                <option value="Ukuran Porsi Mengenyangkan" {{ old('nama_kriteria') == 'Ukuran Porsi Mengenyangkan' ? 'selected' : '' }}>Ukuran Porsi Mengenyangkan</option>
                                <option value="Rating / Ulasan Rasa" {{ old('nama_kriteria') == 'Rating / Ulasan Rasa' ? 'selected' : '' }}>Rating / Ulasan Rasa</option>
                                <option value="Tingkat Kebersihan Tempat" {{ old('nama_kriteria') == 'Tingkat Kebersihan Tempat' ? 'selected' : '' }}>Tingkat Kebersihan Tempat</option>
                            </optgroup>

                            <optgroup label="--- Kategori Aksesibilitas ---" class="font-bold text-saku-primary mt-2">
                                <option value="Jarak ke Warung Makan" {{ old('nama_kriteria') == 'Jarak ke Warung Makan' ? 'selected' : '' }}>Jarak ke Warung Makan</option>
                                <option value="Waktu Penyajian" {{ old('nama_kriteria') == 'Waktu Penyajian' ? 'selected' : '' }}>Waktu Penyajian</option>
                            </optgroup>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" min="0" max="1" id="bobot" name="bobot" value="{{ old('bobot') }}" placeholder="Contoh: 0.30" required
